...
export function RadioPill({ label, isActive, onClick, icon, layout = "row" }: RadioPillProps) {
  return (
    <button
      onClick={onClick}
      className={`flex-1 px-4 rounded-xl border flex items-center justify-center gap-2 transition-all duration-200 ${layout === "col" ? "flex-col py-3" : "py-2.5"
        } ${isActive
...
export function RadioCard({ title, description, isActive, onClick, variant = "default" }: RadioCardProps) {
  ...
  return (
    <button
      onClick={onClick}
      className={`w-full text-left p-4 rounded-xl border transition-all duration-200 flex items-center justify-between group ${isActive ? (isDanger ? dangerActive : defaultActive) : (isDanger ? dangerInactive : defaultInactive)
        }`}
    >
      <div>
        <div
          className={`font-semibold text-sm ${isActive ? (isDanger ? "text-red-600 dark:text-red-400" : "text-foreground") : "text-foreground"
            }`}
        >
          {title}
        </div>
      ...
      {isActive && (
        <svg
          ...
          strokeWidth="2"
          viewBox="0 0 24 24"
          className={`shrink-0 animate-in fade-in zoom-in duration-200 ${isDanger ? "text-red-600 dark:text-red-400" : "text-foreground"
            }`}
        >
