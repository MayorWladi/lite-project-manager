	...
	return (
		<>
			...
			<aside className={`fixed inset-y-0 left-0 z-50 w-64 border-r border-(--color-border) bg-background flex flex-col h-full shrink-0 transition-transform duration-300 ease-in-out md:relative md:translate-x-0 ${isOpen ? 'translate-x-0' : '-translate-x-full'}`}>
				...
				{/* Botón de Ajustes */}
				<div className="p-4 border-t border-(--color-border) flex justify-between items-center">
					<button 
						onClick={() => setIsSettingsOpen(true)}
						className="flex items-center gap-2 text-sm text-(--color-muted) hover:text-foreground transition-colors p-2 -ml-2 rounded-lg hover:bg-black/5 dark:hover:bg-white/5"
					>
